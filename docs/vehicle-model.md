...
## 1c. 积分精度：控制 20 Hz，被控对象 80 Hz

控制（设定点解析、相位判定、速度规划、包络裁剪）仍按 **20 Hz** 跑一次；**被控对象**（姿态速率环、推力矢量、垂直通道、
自由落体、偏航转动）在同一 tick 内**子步 4 次**（12.5 ms 一步）积分。需求速度在一次控制 tick 内保持不变
（零阶保持——真实控制器两次更新之间就是这样）。

为什么不是"一步到位"，这是量出来的，不是感觉出来的（`IntegrationFidelityTest` 会持续守着这些数字）：

| 指标 | 单步 50 ms（旧） | 4×12.5 ms（现在） |
| --- | --- | --- |
| 位置积分的矩形偏差（1.4 m/s 加速段） | **3.5 cm** | **0.875 cm** |
| 速度与细参考解的最大差（1 m/s） | 0.0457 m/s | **0.0100 m/s** |
| 与二阶闭式解的最大差 | 10.5% | **2.4%** |
| 自由落体高度与 ½gt² 的差（0.5 s） | 12.3 cm | **3.1 cm** |
| 松杆后的滑行距离（相对连续解） | 77% | **93%** |

顺带修了一件事：位置跟踪原来按"一个滞后时间"估算刹车距离，而真实刹车要经过姿态与速度**两级**滞后——
旧的运动学对象"一松手就减速"掩盖了这个低估，换成真实对象后它会**冲过目标约 9 cm**。
现在刹车距离按两级滞后估算（并留余量），实测 3 m 点到点**停在 −0.38 cm**并保持。
