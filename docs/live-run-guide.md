...
它只读不写，检查那些会**白白浪费一次上机**的事：

| 检查 | 为什么 |
| --- | --- |
| 刚构建的 JAR 与实例里已装 JAR 的 SHA-256 是否一致 | 最容易踩的坑：构建了新版本却忘了重装，于是测的是旧代码——本项目的首次预检就发现实例里还留着 8/26 的构建 |
| 实例里有 `fabric-api-*.jar` | 缺了模组根本不会加载 |
| 模组本地端口 `14601` 是否空闲 | 被别的东西占着时模组会退回动态端口，后端需要重新学习来源端点 |
| 隔离端口 `18082/14561/18151/18152` 的占用情况 | 全空 = backend 还没起；全占 = 已在跑；只占一部分 = 有残留进程 |
| 生产 backend（`8080`）是否在跑 | 现场实例与隔离实例必须分清，别把前端连到现场后端 |
| 主项目 `drone_backend.exe` 是否已构建 | 隔离脚本会用它 |

任一项 FAIL 时退出码为 1，并给出该执行的命令。确实要故意测旧构建时加 `-AllowStaleMod`。
...
说清楚边界，免得把"脚本全绿"读成"游戏里一定没问题"。

**已被证明的**（都能自己跑）：

| 命令 | 证明什么 |
| --- | --- |
| `node .\scripts\verify-contract.mjs` | 后端那一侧整条会话：发现源 → 信标被接受 → 参数清单 → GUIDED → 起飞（indoor origin/解锁/起飞/爬升）→ PVA 帧 → 机型状态回读 → 降落 → 断开重连转发保持。含两项离线自检：脚本自己重建的模组黄金帧与 `MavlinkV1CodecTest` 的字节逐位一致 |
| `.\gradlew.bat test` | 模组自身的部件与**入站**命令路径：每一种 PVA `type_mask` 经真实编解码器与真实 autopilot 进真实飞行模型；MAVLink v1 负载长度对着官方 `MIN_LEN`；CRC extra 对着后端链接的官方 C 库；控制端点四条指令；信标负载；坐标变换；飞行包络；训练场布局 |
| `.\gradlew.bat closedLoopTest` | 游戏内 `/minidrone selftest` 的那 10 项检查，无头跑一遍 |

**没有被证明的**（诚实清单）：

1. **真人上机那一次**：Minecraft 自己的实体生成与 tick、客户端渲染、`/minidrone` 的聊天输出、PCL 启动。
   本会话有三处缺陷只有读 Minecraft 自身的语义才能发现（`SavedData` 的 data-fix 参数、训练场回执、MAVLink v1 扩展字段），
   而"读代码 / 读字节码"终究只是代理，不是替代。
2. **客户端渲染只做过源码级检查**（部件名、model layer 注册、同步字段、旋转矩阵），没有验证过任何一帧画面；
   侧倾方向的符号尤其无法从源码确定。
3. **传输循环的节奏与套接字生命周期**靠阅读加一条不变量测试保证，没有无头跑过它——它在启动时会经 SLF4J 记录日志，
   而测试运行时不带 SLF4J。
4. **多源/真机并存**按设计不在范围内（见兼容性文档 §1.1）。
