...
## 3.45 俯视：标签改名 + 右键改为取消选择 + 移除右键旋转（2026-09-24）

操作者：①右下标签的"Ctrl 切换"改成更具体的"选择模式"之类；②取消该模式下右键旋转，改为右键取消选择。

### (a) 标签：说"是什么"而不是"按哪个键"

| 模式 | 改前 | 改后 |
| --- | --- | --- |
| 切换·关 | `Ctrl 切换 · 关` | **`选择模式 · 关`** |
| 切换·开 | `Ctrl 切换 · 开` | **`选择模式 · 开`** |
| 按住 | `Ctrl 按住` | **`选择模式 · 按住 Ctrl`** |

理由：它是一个**按钮**，旧文案告诉用户"去按哪个键"，新文案说明"这个模式是什么"，开关状态跟在后面。
tooltip 同步改为《点击开启或关闭选择模式（加选／框选）；也可按 Ctrl 切换》。
...
| 手势 | 原行为 |
| --- | --- |
| 右键单击 | **把选中的飞机移动到你点的地方**（RTS 常用） |
| 右键拖动 | 旋转俯视相机 |

"右键改为取消选择"会与"右键单击移动"**直接冲突**。已确认的方案：**右键专职取消选择，
"移动到这里"改绑左键双击**。
...
| 位置 | 改动 | 说明 |
| --- | --- | --- |
| `view-manager.js` | **删除**右键拖拽旋转的全部机制 | 删除 `_onOrthoMouseDown/Move/Up`、`_addOrthoMouseListeners`、`_removeOrthoMouseListeners` 及相关状态；`_updateOrthoCameraRotation` → **`_applyTopDownPose()`**（不再有角度参数）；`mouseButtons.RIGHT` → **`null`** |
| `selection-manager.js` | 右键单击 → **`_clearSelection()`** | 位移仍走 `shouldDispatchMapClick` 阈值判定，**右键拖动不会触发取消**（也再无旋转） |
| `selection-manager.js` | 新增 **`_onDoubleClick`** → `_dispatchMapCommand` | 仅战术俯视 + 视口内 + **无修饰键**（Shift/Ctrl/Alt 属于加选/框选手势） |
| 文案 | HUD 与反馈同步 | `左键拖动平移 · 左键双击移动 · 滚轮缩放`、`左键单击选择 · 右键取消选择 · …` |

**相机永远轴对齐**：`getTopDownCameraPose(target, height, 0)`，即 world +Z 朝屏幕上方（文档定义的朝向）。
测试同步更新为断言"停在被平移的 target 正上方且 up = world +Z"。

**测试**：新增 4 项——右键单击清空选择且**不再移动**、右键拖动**不**触发取消、双击移动、
修饰键双击无效/非俯视模式无效；并同步改名 2 项名不副实的旧用例。前端 **225/225**。

## 3.46 俯视框选：不是坏了，是被藏起来了（2026-09-24）

操作者："发现俯视角选中的框选没了，请适配"。

### 先实测，别猜

把整个手势链在 node 里跑了一遍（正交相机 + 真实视口判定 + 真实坐标换算），得到行为矩阵：

| 场景 | 结果 |
| --- | --- |
| Shift/Ctrl + 左键拖动 | ✅ 框选正常（出框、出「N 架」、选中正确） |
| 选择模式（切换）**开启后**不按修饰键直接拖动 | ✅ 框选正常 |
| **默认偏好（按住 Ctrl）下点徽标** | ❌ **按钮 `disabled`，点了没反应** |
| 默认偏好下直接拖动 | ❌ 变成平移画面 |

**所以框选逻辑没坏**（`selection-rect` 的 CSS 也一直在 `_selection.css` —— 我先只搜了 `main.css`，
这个"没有样式"的结论是我搜错了文件，已更正）。真正的问题是**可用性**：

- 徽标在默认偏好下是**禁用的**（`indicator.disabled = !isToggleMode`）；
- 而它现在叫"**选择模式**"——一个"模式"意味着"打开它，然后用鼠标操作"，
  但默认下既按不动、拖动又只是平移 ⇒ 用户视角就是"框选没了"。

### 修复：让"选择模式"成为真正的模式

| 位置 | 改动 |
| --- | --- |
| `_toggleCtrlSelectionLatch()` | 去掉 `ctrlSelectionMode !== 'toggle'` 守卫 —— 任何偏好下都可开关（仍要求战术俯视） |
| `_syncCtrlSelectionState()` | `disabled = false` 恒定；文案恒为 `选择模式 · 开/关`；tooltip 说明开启后左键拖动即框选 |
| `_getEffectiveSelectionEvent()` | 不再分偏好：**模式开启 或 按住 Ctrl** 任一成立即视为 Ctrl 生效 |
| `_updateDefaultFeedback()` | 提示随模式变化（开启时说"左键拖动框选"，关闭时说"拖动平移／开启选择模式或按 Shift/Ctrl 框选"） |

偏好（按住 / 切换）现在只管**Ctrl 键本身**的行为（临时按住 vs 点按切换），不再决定徽标能不能按。

实测（两种偏好一致）：

```text
pref=hold   | 徽标 disabled: false | 标签: 选择模式 · 关 → 按下 → 选择模式 · 开
pref=toggle | 徽标 disabled: false | 标签: 选择模式 · 关 → 按下 → 选择模式 · 开
两种情况：不按修饰键直接拖动 → 框选生效 → 选中 ["d-a","d-b"]
```

**测试**：新增 3 项——①按下模式后**不按修饰键直接拖动即框选**（且模式关闭时拖动仍是平移）
②两种偏好下行为一致 ③不开启模式时**按住 Ctrl 仍可框选**（不破坏原有用法）。前端 **228/228**。

## 3.47 俯视下选中框永不出现（真 bug：用"距离"判定正交相机）（2026-09-24）

操作者澄清："我是说选中后被选中的飞机，只有靶心样式的表示，近距离的选中框没了"。
（上一节 §3.46 我把这句误解成"框选手势没了"，那部分改动本身仍是对的，但与本次无关。）

### 根因：样式切换用"相机到飞机的距离"，对正交相机毫无意义

`getSelectionDistanceRatio()` 用的是 `camera.position.distanceTo(mesh.position)`。而俯视是**正交相机**，
它为了框住整个场地必须架得很高：

```text
49 m 场地 -> height = 49 x 1.2 = 58.8 m  -> 距离恒为 58.8 m -> ramp 恒为 1.00（最远）
zoom  1: 视口覆盖 58.80 m -> 飞机 4 px
zoom 10: 视口覆盖  5.88 m -> 飞机 36 px
zoom 20: 视口覆盖  2.94 m -> 飞机 72 px   <- 此时早就该显示选中框了
```

即：**无论放大到多少倍，距离始终是 58.8 m，ramp 永远是 1，样式永远是"远景"** ⇒ 选中框永不出现 ✗。
正交相机的"距离"本来就不影响成像大小，用距离做判据在原理上就是错的。

### 修复：改用"视口在飞机处覆盖多少世界高度"（即视觉大小）

`getViewportSpanAt()` 早就对两种相机都算对了（透视用 `2·d·tan(fov/2)`，正交用 `视锥高 / zoom`）。
把 ramp 改用它：

```js
const SELECTION_RANGE_NEAR_SPAN_M = 2 * 3.0  * tan(60°/2);   // = 3.46 m
const SELECTION_RANGE_FAR_SPAN_M  = 2 * 20.0 * tan(60°/2);   // = 23.09 m
```

阈值按 60° 参考 fov 从原距离换算，所以**透视行为完全不变**；`getSelectionDistanceRatio`
更名 **`getSelectionRangeRatio`**（名字不再撒谎）。
...
| --- | --- | --- |
| 透视（不变） | 6.60 m | **~32 px** |
| 俯视（正交，修复前**永不切换**） | 6.60 m | **~32 px** |

俯视下 zoom ≥ ~12 即切回选中框（zoom 1 时飞机仅 4 px，显示靶心是对的）：

```text
zoom  1 | 58.80 m |  4 px | far  | 靶心
zoom  5 | 11.76 m | 18 px | far  | 靶心
zoom 10 |  5.88 m | 36 px | far  | 靶心
zoom 14 |  4.20 m | 50 px | near | 选中框
zoom 20 |  2.94 m | 72 px | near | 选中框
```

**测试**：新增 1 项"样式切换跟随**视觉大小**、而非相机距离"——分别扫透视距离与俯视 zoom，
取各自切换瞬间的视口覆盖值，断言两者相差 <25%（若按距离判定则二者天差地别），
并断言切换点落在飞机约 20–50 px 处。另修正 2 项因改用真实相机而暴露的旧断言
（其中一项此前靠"假相机没有 fov"绕过了视角上限，已拆成两条独立断言）。前端 **229/229**。
